...
void translateArray(Node* node, Type type, Type tp, Operand* last, Operand* temp)
{
	...
	if(strcmp(q->children[0]->name,"ID")==0)
	{
		...
		ic->u.triop.op1->u.name = strdup(q->children[0]->value);
		ic->u.triop.op2 = t1;
		insertLink(ic);
	}
	else
...
char* translateVarDec(Node* node, int size)
{
	...
	if (node->children_num == 1)
	{
		...
		if (size > 4)
		{
			...
			ic->u.dec.op->u.name = strdup(node->children[0]->value);
			ic->u.dec.size = size;
			insertLink(ic);
...
void translateExp(Node* node, Operand* op)
{
		...
		if (strcmp(node->children[1]->name, "ASSIGNOP") == 0)
		{
			...
			if (it == 0)
			{
				...
				if (op != 0)
				{
					...
					ic->u.binop.op->u.name = strdup(node->children[2]->children[0]->value);
	...
	{
		translateExp(node->children[1], op);
	}
	...
	else if (strcmp(node->children[0]->name, "ID") == 0)
	{
		...
		if (node->children_num == 1)
		{
			...
			ic->u.binop.op = newOperand();
			ic->u.binop.op->kind = VARIABLE_OP;
			ic->u.binop.op->u.name = strdup(node->children[0]->value);
	...
	else if(strcmp(node->children[0]->name, "INT") == 0)
	{
		...
		op->kind = CONSTANT_OP;
		op->u.value = strtol(node->children[0]->value, 0, 10);
		//printf("----%d\n", op->u.value);
		//printf("woo\n");
	}
	else
